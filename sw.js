const VERSION="chama-clean-v115";
  ...
  event.waitUntil((async () => {
    ...
    await Promise.all(windows.map(async client => {
      try {
        ...
        if (url.searchParams.get("chama_update") === "115") return;
        url.searchParams.set("chama_update", "115");
...
function injectSafeUi(html) {
  ...
  const menuMarker = 'app-menu.js?v=5';
  ...
  if (!out.includes(menuMarker)) out = out.replace('</body>', `<script src="./app-menu.js?v=5"></script></body>`);
