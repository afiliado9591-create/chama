const VERSION="chama-clean-v127";
  ...
  event.waitUntil((async () => {
    ...
    await Promise.all(windows.map(async client => {
      try {
        ...
        if (url.searchParams.get("chama_update") === "127") return;
        url.searchParams.set("chama_update", "127");
...
function injectSafeUi(html) {
  ...
  const unreadMarker = 'unread-badges.js?v=4';
  ...
  if (!out.includes(unreadMarker)) out = out.replace('</body>', `<script src="./unread-badges.js?v=4"></script></body>`);
