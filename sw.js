const VERSION="chama-clean-v176";
  ...
  event.waitUntil((async () => {
    ...
    await Promise.all(windows.map(async client => {
      try {
        ...
        if (url.searchParams.get("chama_update") === "176") return;
        url.searchParams.set("chama_update", "176");
...
function injectSafeUi(html) {
  ...
  const affiliateMenuMarker = 'affiliate-menu.js?v=7';
  ...
  if (!out.includes(affiliateMenuMarker)) out = out.replace('</body>', `<script src="./affiliate-menu.js?v=7"></script></body>`);
