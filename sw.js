const VERSION="chama-clean-v114";
  ...
  event.waitUntil((async () => {
    ...
    await Promise.all(windows.map(async client => {
      try {
        ...
        if (url.searchParams.get("chama_update") === "114") return;
        url.searchParams.set("chama_update", "114");
...
function injectSafeUi(html) {
  ...
  const menuMarker = 'app-menu.js?v=4';
  ...
  if (!out.includes(menuMarker)) out = out.replace('</body>', `<script src="./app-menu.js?v=4"></script></body>`);
