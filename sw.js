const VERSION="chama-clean-v116";
  ...
  event.waitUntil((async () => {
    ...
    await Promise.all(windows.map(async client => {
      try {
        ...
        if (url.searchParams.get("chama_update") === "116") return;
        url.searchParams.set("chama_update", "116");
...
function injectSafeUi(html) {
  ...
  const menuMarker = 'app-menu.js?v=6';
  ...
  const adminPagesMenuMarker = 'admin-pages-menu.js?v=1';
  let out = html;
  ...
  if (!out.includes(menuMarker)) out = out.replace('</body>', `<script src="./app-menu.js?v=6"></script></body>`);
  ...
  if (!out.includes(adminPagesMenuMarker)) out = out.replace('</body>', `<script src="./admin-pages-menu.js?v=1"></script></body>`);
  return out;
}
