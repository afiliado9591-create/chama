...
String.prototype.startsWith=function(search,...args){
  ...
  if(search===PREFIX && value.startsWith(PREFIX)){
    try{
...
function mediaBubble(b,m){
  ...
  b.dataset.mediaBridge='1';
  b.dataset.chamaRaw=m.raw;
...
function repair(){
  ...
  while(queue.length&&targets.length){
    const b=targets.shift(),m=queue.shift();mediaBubble(b,m)
  }
}
function watch(){
  const box=$('messages');
  if(box)new MutationObserver(()=>setTimeout(repair,0)).observe(box,{childList:true,subtree:true,characterData:true});
