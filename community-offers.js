(()=>{
  ...
  async function open(){
    ...
    document.body.appendChild(back);back.querySelector('.chama-community-close').onclick=()=>back.remove();back.addEventListener('click',e=>{if(e.target===back)back.remove()});
    if(document.body.dataset.profileType==='profissional'){back.querySelector('#chamaCommunitySave').onclick=()=>saveOwn(back);const own=await readOwn();if(document.body.contains(back))setEditorValues(back,own)}else back.querySelector('.chama-community-editor')?.remove();
    if(document.body.contains(back))loadFeed(back,false);
  }
