(()=>{
  ...
  function addStyle(){
    ...
    const s=document.createElement('style');s.id=STYLE_ID;s.textContent=`
      ...
      .chama-community-picker{position:fixed;inset:0;background:#0009;z-index:3900;display:grid;place-items:center;padding:12px}.chama-community-picker-card{width:min(440px,100%);max-height:80dvh;display:flex;flex-direction:column;background:#fff;border-radius:20px;overflow:hidden}.chama-community-picker-head{padding:13px 14px;background:#0b7a53;color:#fff;display:flex;align-items:center;gap:8px}.chama-community-picker-head strong{flex:1}.chama-community-picker-close{border:0;background:#ffffff20;color:#fff;width:38px;height:38px;border-radius:10px;font-size:19px}.chama-community-picker-list{overflow:auto;padding:8px}.chama-community-person{width:100%;border:0;border-bottom:1px solid #edf1ef;background:#fff;padding:11px 10px;display:flex;align-items:center;gap:10px;text-align:left}.chama-community-person-avatar{width:40px;height:40px;border-radius:50%;background:#e4f3eb;color:#0b7a53;display:grid;place-items:center;font-weight:900}.chama-community-person-name{font-weight:850;color:#24372e}.chama-community-person small{display:block;color:#748079;margin-top:2px}.chama-community-picker-empty{padding:22px 14px;text-align:center;color:#68766f;line-height:1.45}
  ...
  function shareInsideChama(item){
    ...
    const wrap=document.createElement('div');wrap.id='chamaCommunityPicker';wrap.className='chama-community-picker';wrap.innerHTML='<section class="chama-community-picker-card"><header class="chama-community-picker-head"><strong>Enviar produto para</strong><button type="button" class="chama-community-picker-close">✕</button></header><div class="chama-community-picker-list"></div></section>';const list=wrap.querySelector('.chama-community-picker-list');if(!people.length)list.innerHTML='<div class="chama-community-picker-empty">Você ainda não possui uma conversa para compartilhar.<br>Inicie uma conversa e tente novamente.</div>';for(const person of people){const b=document.createElement('button');b.type='button';b.className='chama-community-person';b.innerHTML='<span class="chama-community-person-avatar"></span><span><span class="chama-community-person-name"></span><small>Enviar nesta conversa</small></span>';b.querySelector('.chama-community-person-avatar').textContent=(person.nome||'U').charAt(0).toUpperCase();b.querySelector('.chama-community-person-name').textContent=person.nome;b.onclick=async()=>{if(!me||!db||!fs)return alert('Entre novamente no Chama para enviar.');b.disabled=true;try{const ids=[me.uid,person.uid].sort(),chatId=ids.join('_'),chatRef=fs.doc(db,'chats',chatId),message=shareText(item),label=`🛍️ ${item.title}`;await fs.setDoc(chatRef,{participants:ids},{merge:true});await fs.addDoc(fs.collection(db,'chats',chatId,'messages'),{text:message,senderId:me.uid,receiverId:person.uid,createdAt:fs.serverTimestamp()});await fs.setDoc(chatRef,{participants:ids,lastMessage:label,lastSenderId:me.uid,updatedAt:fs.serverTimestamp(),unreadCounts:{[person.uid]:fs.increment(1),[me.uid]:0}},{merge:true});wrap.remove();document.getElementById('chamaCommunityOffersModal')?.remove();if(typeof window.chamaOpenChat==='function')window.chamaOpenChat(person);alert('Produto enviado na conversa ✓')}catch(e){console.error(e);alert('Não foi possível enviar o produto agora.');b.disabled=false}};list.appendChild(b)}wrap.querySelector('.chama-community-picker-close').onclick=()=>wrap.remove();wrap.onclick=e=>{if(e.target===wrap)wrap.remove()};document.body.appendChild(wrap)
  }
