(()=>{
  const STYLE_ID='chamaUnreadStyleV2';
  let meUid='';
  let db=null;
  let firestore=null;
  ...
  async function waitForBaseList(){
    ...
    while(Date.now()<end){
      ...
      if(list&&!/Carregando pessoas/i.test(list.textContent||''))return list;
  ...
  async function loadProfiles(ids){
    ...
    for(let i=0;i<unique.length;i+=30){
      ...
      try{
        ...
        const found=new Set();
        snap.forEach(d=>{const x=d.data()||{};found.add(d.id);profiles.set(d.id,{uid:d.id,nome:String(x.nome||'Usuário'),email:'',photoUrl:safePhotoUrl(x.photoUrl||'')})});
        batch.forEach(uid=>{if(!found.has(uid))missing.push(uid)});
      }catch{missing.push(...batch)}
    }
    for(let i=0;i<missing.length;i+=30){
      const batch=missing.slice(i,i+30);
      try{
        const snap=await firestore.getDocs(firestore.query(firestore.collection(db,'users'),firestore.where(firestore.documentId(),'in',batch)));
        snap.forEach(d=>{const x=d.data()||{};profiles.set(d.id,{uid:d.id,nome:String(x.nome||x.email||'Usuário'),email:String(x.email||''),photoUrl:''})});
      }catch(_){ }
    }
    unique.forEach(uid=>{if(!profiles.has(uid))profiles.set(uid,{uid,nome:'Usuário',email:'',photoUrl:''})});
  }

  async function ensureAllConversationRows(){
    await waitForBaseList();
    const ids=[...conversations.keys()];
    await loadProfiles(ids);
    ids.forEach(ensureConversationRow);
  ...
  async function resetUnread(row){
    ...
    renderRow(row);
    try{
      await firestore.updateDoc(firestore.doc(db,'chats',c.chatId),{[`unreadCounts.${meUid}`]:0});
    }catch(e){
  ...
  async function loadConversations(user){
    ...
    try{
      ...
      db=fs.getFirestore(found.app);firestore=fs;
      const q=fs.query(fs.collection(db,'chats'),fs.where('participants','array-contains',meUid));
      const snap=await fs.getDocs(q);
      conversations.clear();profiles.clear();
      snap.forEach(d=>{
        const data=d.data()||{},participants=Array.isArray(data.participants)?data.participants:[],otherUid=participants.find(uid=>uid&&uid!==meUid);
        if(!otherUid)return;
        conversations.set(otherUid,{chatId:d.id,unread:countValue(data),lastMessage:data.lastMessage||'',updatedAt:timeValue(data.updatedAt)});
      });
      await ensureAllConversationRows();renderAll();sortConversationRows();
  ...
  async function start(){
    ...
    try{
      ...
      authMod.onAuthStateChanged(auth,user=>{
        if(!user){meUid='';db=null;firestore=null;conversations.clear();profiles.clear();renderAll();return}
