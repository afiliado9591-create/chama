(()=>{
  const STYLE_ID='chamaHomeSelfProfileClickStyleV6';
  ...
  function loadSegments(){
    if(document.querySelector('script[data-chama-segment-groups-v6="1"]'))return;
    const s=document.createElement('script');s.src='./segment-groups-ui.js?v=6';s.async=true;s.dataset.chamaSegmentGroupsV6='1';document.head.appendChild(s);
  }
  function loadForceGroups(){
    document.querySelectorAll('script[src*="groups-force-ui.js"]').forEach(s=>s.remove());
    const s=document.createElement('script');s.src='./groups-force-ui.js?v=3';s.async=true;s.dataset.chamaGroupsForce='3';document.head.appendChild(s);
  }
  function start(){
    if(document.getElementById(STYLE_ID))return;
    const s=document.createElement('style');s.id=STYLE_ID;s.textContent='.chama-self-row{cursor:pointer!important}.chama-self-row:active{background:#eef7f2!important}#chamaSuggestions,.chama-suggestions{display:none!important}#chamaGroupsForceBox,#chamaGroupsForceBox *{pointer-events:auto!important}#chamaGroupsForceBox .gf-row,#chamaGroupsForceBox .gf-enter{pointer-events:auto!important;touch-action:manipulation!important}';document.head.appendChild(s);
