...
function keepLatestMessageVisible(){
  ...
  if(!box)return;
  const reserve=Math.max(42,Math.ceil(composer?.getBoundingClientRect?.().height||0)+24);
  box.style.paddingBottom=reserve+'px';
  box.style.scrollPaddingBottom=reserve+'px';
  const run=()=>{box.scrollTop=box.scrollHeight;const last=box.lastElementChild;if(last)last.scrollIntoView?.({block:'nearest',inline:'nearest',behavior:'auto'});box.scrollTop=box.scrollHeight};
  requestAnimationFrame(()=>{run();setTimeout(run,60);setTimeout(run,180);setTimeout(run,400)});
}
function watchMessages(){
  const box=$('messages');
  if(!box||watcher)return;
  watcher=new MutationObserver(()=>keepLatestMessageVisible());
  watcher.observe(box,{childList:true});
}
