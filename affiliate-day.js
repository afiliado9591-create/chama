(()=>{
  ...
  async function openCatalog(){
    ...
    try{await loadActive(false);await loadCatalog(false)}catch(e){console.error(e);return alert('Não foi possível abrir o catálogo agora.')}
    ...
    if(isToday()&&me.uid===active.affiliateUid){const box=document.createElement('div');box.className='chama-affday-linkbox';const label=document.createElement('label');label.textContent='Seu link de afiliado para o produto que escolher';const input=document.createElement('input');input.id='chamaAffdayCustomLink';input.type='url';input.maxLength=900;input.placeholder='https://seu-link-de-afiliado...';box.append(label,input);body.appendChild(box)}
