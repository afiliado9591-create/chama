(()=>{
  ...
  async function openCatalog(){
    ...
    const canChoose=isToday()&&me.uid===active.affiliateUid&&document.body.dataset.profileType==='profissional';
    if(canChoose)status.innerHTML=`<strong>Hoje é o seu dia.</strong><br>Escolha qualquer produto abaixo, cole o seu link de afiliado e ele ficará em destaque no topo do Chama durante o dia.`;
    ...
    if(!products.length){const empty=document.createElement('div');empty.className='chama-affday-empty';empty.textContent='O catálogo ainda não tem produtos disponíveis.';body.appendChild(empty)}
      ...
      for(const p of products){
        ...
        const actions=document.createElement('div');actions.className='chama-affday-actions';if(p.defaultUrl){const a=document.createElement('a');a.className='chama-affday-link';a.href=p.defaultUrl;a.target='_blank';a.rel='noopener noreferrer sponsored';a.textContent='Ver oferta';actions.appendChild(a)}if(canChoose){const choose=document.createElement('button');choose.type='button';choose.className='chama-affday-choose';choose.textContent=selection?.productId===p.id&&selection?.date===active.date?'✓ Escolhido hoje':'Escolher este produto';choose.onclick=()=>chooseProduct(p);actions.appendChild(choose)}cb.appendChild(actions);card.appendChild(cb);grid.appendChild(card)
      }
