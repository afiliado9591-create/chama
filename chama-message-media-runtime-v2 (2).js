...
let fs=null,authMod=null,db=null,auth=null,ready=false,recording=false,recorder=null,chunks=[],stream=null,watcher=null,composerWatched=false;
...
function enforceComposerLayout(){
  ...
  // Remove qualquer ícone de câmera de foto ou vídeo, venha de onde vier.
  [...form.querySelectorAll('button')].forEach(b=>{
    const t=(b.textContent||'').trim();
    if(t==='📷'||t==='🎥')b.remove();
  });
  // Mantém só um microfone (preferindo o #audioBtn oficial).
  const mics=[...form.querySelectorAll('button')].filter(b=>(b.textContent||'').trim()==='🎤');
  let mic=mics.find(b=>b.id==='audioBtn')||mics[0]||null;
  mics.forEach(b=>{if(b!==mic)b.remove()});
  if(mic&&!mic.id)mic.id='audioBtn';
  // Garante que o clipe existe.
  let clip=$('chamaClipBtn');
  if(!clip){addClip();clip=$('chamaClipBtn')}
  // Ordem final: clipe, microfone, campo de texto.
  if(clip&&mic){form.insertBefore(clip,mic);form.insertBefore(mic,input)}
  else if(clip){form.insertBefore(clip,input)}
}
function watchComposer(){
  const form=$('composer');
  if(!form||composerWatched)return;
  composerWatched=true;
  new MutationObserver(()=>enforceComposerLayout()).observe(form,{childList:true});
}
function addClip(){
  const form=$('composer'),input=$('messageInput');
...
function bind(){enforceComposerLayout();watchComposer();watchMessages();keepLatestMessageVisible();const form=$('composer');if(form&&!form.dataset.chamaSendV4){form.dataset.chamaSendV4='1';form.addEventListener('submit',sendText,true)}const audio=$('audioBtn');if(audio&&!audio.dataset.chamaAudioV4){audio.dataset.chamaAudioV4='1';audio.onclick=e=>{e.preventDefault();e.stopImmediatePropagation();toggleAudio()}}}
