...
function refreshMenu(){const menu=document.getElementById('chamaMobileMenu'),btn=document.getElementById('chamaMobileMenuBtn');if(menu?.classList.contains('open')&&btn){btn.click();setTimeout(()=>btn.click(),20)}}
...
function directButton(u){if(!u||document.getElementById('adminBtn'))return;const top=document.querySelector('.topbar');if(!top)return;const b=document.createElement('button');b.id='adminBtn';b.className='admin-btn';b.textContent='🛡️ Admin';b.type='button';b.onclick=async()=>{b.disabled=true;try{await loadPanel();await new Promise(r=>setTimeout(r,120));const m=document.getElementById('adminModal');if(m){m.classList.remove('hidden');return}const real=document.getElementById('adminBtn');if(real&&real!==b){real.click();return}alert('Não consegui abrir o painel Admin agora.')}finally{b.disabled=false}};top.insertBefore(b,document.getElementById('logoutBtn')||null);refreshMenu()}
async function ensureAdmin(){if(busy)return;const u=auth.currentUser;if(!u)return;busy=true;try{if(!(await adminAllowed(u)))return;await loadPanel();if(!document.getElementById('adminBtn'))directButton(u);refreshMenu()}finally{busy=false}}
function start(u){clearInterval(timer);if(!u)return;ensureAdmin();setTimeout(ensureAdmin,200);setTimeout(ensureAdmin,800);timer=setInterval(ensureAdmin,2500)}
onAuthStateChanged(auth,start);
new MutationObserver(()=>{const u=auth.currentUser;if(u&&!document.getElementById('adminBtn'))setTimeout(ensureAdmin,40)}).observe(document.body,{childList:true,subtree:true});
document.addEventListener('click',e=>{if(e.target.closest('#chamaMobileMenuBtn'))setTimeout(ensureAdmin,0)},true);
