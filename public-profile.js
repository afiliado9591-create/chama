...
async function renderPublic(){
  ...
  try{const s=await getDoc(doc(db,'publicProfiles',uid));if(!s.exists()){root.innerHTML='<div class="public-card"><p>Este perfil ainda não está disponível publicamente.</p><p>Peça ao usuário para abrir o Chama uma vez e tente novamente.</p></div>';return}const p=s.data(),items=Array.isArray(p.catalogo)?p.catalogo:[];
    ...
    else if(catalogo)root.innerHTML=`<div class="public-card"><a class="public-back" href="${location.origin+location.pathname}?perfil=${encodeURIComponent(uid)}">← Ver perfil</a>${profileHeader(p)}<h2>Catálogo</h2><div class="public-grid">${items.length?items.map(productCard).join(''):'<p>Nenhum produto cadastrado.</p>'}</div></div>`;
    else{const item=items[index];if(!item){root.innerHTML='<div class="public-card">Produto não encontrado.</div>';return}root.innerHTML=`<div class="public-card"><a class="public-back" href="${location.origin+location.pathname}?catalogo=${encodeURIComponent(uid)}">← Ver catálogo</a>${profileHeader(p)}<div class="public-grid" style="grid-template-columns:minmax(0,360px);justify-content:center">${productCard(item)}</div></div>`}
