...
export async function onRequestPost({request,env}){
 try{
  ...
  const idToken=auth.slice(7),body=await request.json(),callId=String(body.callId||''),calleeId=String(body.calleeId||'');if(!callId||!calleeId)return new Response('bad request',{status:400});
  const project=env.FIREBASE_PROJECT_ID||'chama-cfc28';
  const call=await firestoreGet(project,'calls/'+encodeURIComponent(callId),idToken);const callerId=field(call.fields?.callerId),storedCallee=field(call.fields?.calleeId),status=field(call.fields?.status);
  if(!callerId||storedCallee!==calleeId||status!=='ringing')return new Response('forbidden',{status:403});
  const user=await firestoreGet(project,'users/'+encodeURIComponent(calleeId),idToken);const primary=field(user.fields?.pushToken);const list=field(user.fields?.pushTokens)||[];const tokens=[...new Set([primary,...(Array.isArray(list)?list:[])].filter(Boolean))].slice(0,12);
  ...
  const access=await serviceToken(env),callerName=String(body.callerName||field(call.fields?.callerName)||'Alguém').slice(0,80),origin=new URL(request.url).origin,link=origin+'/?call='+encodeURIComponent(callId);let sent=0,errors=[];
  for(const token of tokens){
    const payload={message:{token,data:{type:'chama_call',callId,callerId:String(callerId),callerName},webpush:{headers:{Urgency:'high',TTL:'90'},notification:{title:'📞 Chamada no Chama',body:callerName+' está ligando para você',icon:origin+'/icon.svg',badge:origin+'/icon.svg',tag:'chama-call-'+callId,renotify:true,requireInteraction:true,vibrate:[900,250,900,250,900,250,900],actions:[{action:'open',title:'Atender'},{action:'dismiss',title:'Recusar'}],data:{type:'chama_call',callId}},fcm_options:{link}}}};
    ...
    if(r.ok)sent++;else errors.push({status:r.status,body:(await r.text()).slice(0,300)});
  }
 ...
 }catch(e){console.error('call push',e);return new Response(JSON.stringify({sent:0,reason:e?.message||'erro'}),{status:500,headers:{'content-type':'application/json'}})}
}
